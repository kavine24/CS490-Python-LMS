<?php
  ...
  if($_POST['requestType'] == "LoginRequestFTM"){
    ...
    $questionType = ($questionType == 'forLoop') ? 1 : (($questionType == 'nestedForLoop') ? 2 : 3);
    ...
    $post_data = array('difficulty' => $difficulty, 'questionType' => $questionType, 'prompt' => $prompt, 
      'inputvals' => $inputvals, 'outputvals'=> $outputvals, 'requestType' => 'CreateQuestionMTB');
    ...
    $url = 'http://cs490testcenter.ddns.net:50000/back/backDatabase.php';
    $post_data = array("examName" => $examName, "questionIDs" => $questionIDs, "numPoints" => $numPoints, "requestType" => "CreateQuizMTB");
    ...
    for ($i = 0; $i < count($studentAnswers); $i++)
    {
      ...
      for ($j = 0; $j < count($cases); $j++)
      {
        if($studentAnswers[$i] -> questionID == $cases[$j] -> questionID){
          ...
          //compare answers now
          $score += (strcmp($cases[$j] -> output, $output[0]) == 0) ? 1 / ($isNotDumb == 0 ? 1 : 2): 0;
          $counter++;
        }
      }
      array_push($newGrades, array("studentAnswersID" => $studentAnswers[$i] -> studentAnswersID, 
        "pointsCorrect" => (($score/$counter) * ($studentAnswers[$i] -> difficulty == 1 ? 5 : ($studentAnswers[$i] -> difficulty == 2 ? 10 : 15)))));
    }
