<?php
  ...
  else if ($_POST['requestType'] == 'GetStudentsAnswersMTB')
  {
    ...
    $getAnswers = 
    "SELECT A.studentExamsID, A.questionID, Q.questiontype, Q.difficulty, Q.question, A.answer, A.pointsCorrect, E.points, A.comments, A.headerCorrect, A.constraintsCorrect " .
    ...
    while ($row = $pointer2->fetch_array())
    {
      ...
      array_push($answers, array("studentExamsID" => $row[0], "questionID" => $row[1], "type" => $row[2], 
        ...
        "points" => $row[7], "comments" => $row[8], "headerCorrect" => $row[9], "constraintsCorrect" => $row[10], "pointsTable" => $pointsTable));
    }
  ...
  else if($_POST['requestType'] == 'GetAllOfExamMTB')
  {
    ...
    "INNER JOIN (Select * from examQuestions where examID in (SELECT examID FROM exams WHERE examName = 'Strings')) as E ON studentAnswers.questionID = E.questionID " . 
    "WHERE studentExamsID IN (SELECT studentExamsID FROM studentExams " . 
    "WHERE examID IN (SELECT examID FROM exams WHERE examName = 'Strings'));"
    ...
    while ($row1 = $pointer1->fetch_array())
    {
      array_push($examArray, array("studentAnswersID" => $row1[0], "studentExamsID" => $row1[1], 
        ...
        "comments" => $row1[5], "difficulty" => $row1[6], "f" => $row1[7], "w" => $row1[8], "r" => $row1[9]));
    }
