<?php
  ...
  if($_POST['requestType'] == "LoginRequestFTM"){
    ...
    for ($i = 0; $i < count($studentAnswers); $i++)
    {
      ...
      $fconst = $f ? strpos($studentfunctionBody, 'for') != -1 : 1;
      $wconst = $w ? strpos($studentfunctionBody, 'while') != -1 : 1;
      $rconst = $r ? strpos($studentfunctionBody, substr($sanswer,4,strpos($sanswer, '(') - 4)) != -1 : 1;

      print_r(strpos($studentfunctionBody, 'for') === FALSE);
      // print_r($wconst);
      // print_r($rconst);
      // print_r(" ");
