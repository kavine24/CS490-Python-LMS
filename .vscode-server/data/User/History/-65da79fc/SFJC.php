<?php
  ...
  if($_POST['requestType'] == "LoginRequestFTM"){
    ...
    for ($i = 0; $i < count($studentAnswers); $i++)
    {
      ...
      $fconst = $f ? (strpos($studentfunctionBody, 'for') === FALSE ? 0 : 1): 1;
      $wconst = $w ? (strpos($studentfunctionBody, 'while') === FALSE ? 0 : 1): 1;
      $rconst = $r ? (strpos($studentfunctionBody, substr($sanswer,4,strpos($sanswer, '(') - 4)) === FALSE ? 0 : 1): 1;
      ...
      for ($j = 0; $j < count($cases); $j++)
      {
        if($studentAnswers[$i] -> questionID == $cases[$j] -> questionID){
          ...
          //compare answers now
          $result = (strcmp($cases[$j] -> output, $output[0]) == 0) ? 1: 0;

          array_push($pointsTable, array("studentAnswersID" => $studentAnswers[$i] -> studentAnswersID, 
            "caseID" => $cases[$j] -> caseID, "output" => $output[0], "pointsEarned" => 0));

          $score += $result;
          $counter++;
        }
        $pointsTable[count($pointsTable) - 1]['pointsEarned'] = (($result / $counter) * (0.8 * $studentAnswers[$i] -> points));
      }

      $finalScore = (($score / $counter) * (0.8 * $studentAnswers[$i] -> points)) 
      + ((0.1 * $studentAnswers[$i] -> points) * $consts) 
      + ((0.1 * $studentAnswers[$i] -> points) * $header);

      array_push($newGrades, array("studentAnswersID" => $studentAnswers[$i] -> studentAnswersID, 
        "pointsCorrect" => $finalScore, "consts" => $consts, "header" => $header));
    }
