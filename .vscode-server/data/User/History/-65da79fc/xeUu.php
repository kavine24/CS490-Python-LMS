<?php
  ...
  if($_POST['requestType'] == "LoginRequestFTM"){
    ...
    for ($i = 0; $i < count($studentAnswers); $i++)
    {
      ...
      $fconst = $f ? (strpos($studentfunctionBody, 'for')?1:0): 1;
      $wconst = $w ? strpos($studentfunctionBody, 'while'): 1;
      $rconst = $r ? strpos($studentfunctionBody, substr($sanswer,4,strpos($sanswer, '(') - 4)): 1;
