<?php
  ...
  if($_POST['requestType'] == "LoginRequestFTM"){
    ...
    for ($i = 0; $i < count($studentAnswers); $i++)
    {
      ...
      $fconst = strpos($sanswer, 'for') != -1;
      $wconst = strpos($sanswer, 'while') != -1;
      $rconst = strpos($sanswer, substr($sanswer,4,strpos($sanswer, '(') - 4)) != -1;

      print_r(substr($sanswer,4,strpos($sanswer, '(') - 4));
