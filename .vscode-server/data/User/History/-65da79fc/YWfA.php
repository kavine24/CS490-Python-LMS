<?php
  ...
  if($_POST['requestType'] == "LoginRequestFTM"){
    ...
    for ($i = 0; $i < count($studentAnswers); $i++)
    {
      ...
      $fconst = $f ? (strpos($studentfunctionBody, 'for') === FALSE ? 0 : 1): 0;
      $wconst = $w ? (strpos($studentfunctionBody, 'while') === FALSE ? 0 : 1): 0;
      $rconst = $r ? (strpos($studentfunctionBody, substr($sanswer,4,strpos($sanswer, '(') - 4)) === FALSE ? 0 : 1): 0;
