<?php
  ...
  else if ($_POST['requestType'] == 'CreateQuestionMTB')
  {
    ...
    {
      $addQuest = "INSERT INTO questions (question, difficulty, questiontype, f, w, r) VALUES ('$question', '$difficulty', '$questType', '$constraints['for']', " . 
      " '$constraints['while']', '$constraints['recursive']');";
  ...
  else if ($_POST['requestType'] == 'CreateQuizMTB')
  {
    ...
    if ($pointer->num_rows == 0)
    {
      ...
      {
        $addExamQuestions = "INSERT INTO examQuestions (examID, questionID) VALUES ($examID, $questionids[$i]);";
  ...
  {
    $examID = $_POST['examID'];
    $getExamName = "SELECT examName FROM exams WHERE examID = $examID";
    $pointer = $db->query($getExamName);

    $questions = array();
    array_push($questions, $pointer->fetch_array());

    $getExam = "SELECT * FROM questions WHERE questions.questionID IN (SELECT questionID FROM examQuestions WHERE examID = $examID)";
