<?php
  ...
  else if ($_POST['requestType'] == 'ChangeGradeAndCommentsMTB')
  {
    ...
    for ($i = 0; $i < count($gradeChangesFormatted); $i++)
    {
      ...
      "WHERE studentExamsID = $studentExamsID AND questionID = $questionID) + $totalPoints " .
      ...
      "SET pointsCorrect = $totalPoints, fCorrect = $fCorrect, wCorrect = $wCorrect, rCorrect = $rCorrect, comments = '$comments' " .
      ...
      for ($j = 0; $j < count($newPointsTable); $j++){
        $pointsEarned = $newPointsTable['pointsEarned'];
        $pointsTableID = $newPointsTable['pointsTableID']
